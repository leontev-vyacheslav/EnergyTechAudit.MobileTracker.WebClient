...
function TrackMapLocationRecordsProvider (props) {
    ...
    const { getLocationRecordsByRangeAsync, getGeocodedAddressAsync, getGeocodedAddressesAsync } = useAppData();
    ...
    useEffect(() => {
        ...
        (async  () => {
            if (isLoaded && isShowStationaryZone && trackLocationRecordList && trackLocationRecordList.length > 0) {
                ...
                console.log('setStationaryClusterList');
                setStationaryClusterList([...currentStationaryClusterList]);
            }
        })();

    }, [clearOverlays, getBoundsByMarkers, getGeocodedAddressesAsync, isLoaded, isShowStationaryZone,
        stationaryZoneCriteriaAccuracy, stationaryZoneCriteriaSpeed, stationaryZoneElementCount, stationaryZoneRadius,
        trackLocationRecordList, useStationaryZoneAddresses, useStationaryZoneCriteriaAccuracy])

    useEffect(() => {
        if (!isShowStationaryZone) {
            clearOverlays();
        }
    }, [clearOverlays, isShowStationaryZone]);
